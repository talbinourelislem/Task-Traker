...
export default function Map2() {
  ...
  try {
    const record = await pb.collection('worker').getList();
    console.log("recorddd", record);
    record.items.forEach(item => {
      // Call createUserLocationMarker for each record item
      createUserLocationMarker({
        latitude: item.latitude,
        longitude: item.longitude,
        description: item.username // or any other description you want to display
      });
    });
  } catch (error) {
...
const createUserLocationMarker2 = useCallback(
    ({ latitude, longitude, description }) => {
      try {
        // Check if mapRef is available
        if (mapRef && mapRef.current) {
...
const description = ''; // You can change the description as needed
